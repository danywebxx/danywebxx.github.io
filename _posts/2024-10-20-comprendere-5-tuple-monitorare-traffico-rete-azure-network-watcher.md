...
description: Non esiste sicurezza se non si ha visibilità; sia traffico di rete tra endpoint o accesso fisico al datacenter; questo concetto va impresso a fuoco nella mente. Quando parliamo di traffico di rete e sicurezza, i **5-tuple** sono fondamentali per comprendere come il flusso di dati viene indirizzato tra i dispositivi. In questo articolo, esploreremo cosa sono i 5-tuple, come abilitare Network Watcher in Azure per monitorare il traffico e come raccogliere e analizzare i dati attraverso Log Analytics.
image:
...
Come tipologia seleziona **Virtual Network** (i flow log per gli NSG saranno presto deprecati)  
...
Se svogliate lo storage-account creato nella prima parte comincerete a vedere dei dati raccolti e archiviati per **mac-address** e per giorno in formato **json**.
